...
public class GameManager extends JFrame implements KeyListener {
	...
	private static final long serialVersionUID = 1027435907668039188L;
	...
	public static void main(String[] args) {
		...
		// of values
		GameManager managerObi = new GameManager(1920, 128, 30);
	}
	...
	public GameManager(int preferredWidth, int preferredHeight, int maxEnemies) { 
			  ...
			  // Create a Stage object to hold the background 
			  images this.stage = new Stage();
			  ...
			  for (int i = 0; i < this.numEnemies; i++) {
				        this.enemies[i] = new Enemy(this, Math.abs(rng.nextInt()) % (this.canvasWidth / 4),
				        				        Math.abs(rng.nextInt()) % this.canvasHeight); this.gameGraphics = getGraphics();				        				      
			  }
	...
	public void updateCanvas() {
		...
		// If the player is alive, this should move the player in the direction of the // key that has been pressed
		...
		// If an enemy is close to the goal, the player and goal die int j=0;
		int j;
